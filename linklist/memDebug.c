/*#########################################################################
  ##  FILENAME: memDebug.c
  ##
  ##  DESC: catch memory leak when calling malloc(), memset(), free()
  ##  USAGE:
  ##  IMP NOTE:
  ##  COMPILATION:
...
/*=========================================================================
  ==  @ crop_assert_nohang @
  ==
  ==  DESC: for non-critical error checking, app can continue even error occur
  ==  USAGE:
  ==  INPUTS:
  ==  OUTPUTS:
  ==  RETURN:
...
void throwErr(int line, const char * fileId, memLogObj * pMemObj)
{
   ...
   printf("Origin from %s:%d, gDebugMsg(%s), gDebugMsgValue(%p)\n", gDebugSrc, 
		   gDebugLine, (char *) gDebugMsg, gDebugMsgValue[0]);
   /*
   if(gDebugMsgValue[0]!=NULL && crop_memChkData(gDebugMsgValue[0], 
   &gDebugMemObj))
   {
   ...
   */
   if(pMemObj != NO_LOG)
   {
      ...
      {
	 printf("throwErr: (8)FENCE_TOP+(8)ByteAf: \n");
	 crop_memPrintData(pMemObj->ptr, 16);
	 printf("throwErr: (8)ByteB4+(8)FENCE_BOTTOM: \n");
	 crop_memPrintData(pMemObj->ptr_end-8, 16);
      }
   }
}

int addNode(memLogObj * newNode, int line, const char * fileId, int thread_id, 
		int pid)
{
   ...
   //line, thread_id, pid, newNode->ptr_start);
 ...
 * to be deleted 
 * *****************************************************************/
int removeNode(memLogObj * p_Prev, memLogObj * p_ToBeDeleted, 
		int line, const char * fileId)
{
	...
	{
		CROP_ASSERT(p_ToBeDeleted==p_Prev && p_ToBeDeleted==pMemHead && 
				p_ToBeDeleted==pMemTail, __LINE__, __FILE__, p_ToBeDeleted);
		if(p_Prev->isArray==0)
		{
	...
	else
	{
		...
		{
			if(B_YES==p_ToBeDeleted->toBeMemset)
			{
		...
		const char * fileId, int thread_id, int pid)
{
   ...
   pthread_mutex_lock(&mutexRecursive);
   //---------------------------------------------- copy objName//
   CROP_ASSERT(30>=strlen(objName), line, fileId, NO_LOG);
   char * str_name = (char *) malloc(strlen(objName)+1);
   strcpy(str_name, objName);

   ptr = (unsigned char *)malloc(size+FENCE_BYTE_TOTAL);
      ...
      int size, char * objName, int line, const char * fileId)
{
   ...
   CROP_ASSERT(pDest != NULL, line, fileId, NO_LOG);
   CROP_ASSERT(pSrc != NULL, line, fileId, NO_LOG);
   CROP_ASSERT(size > 0, line, fileId, NO_LOG);
   ...
   for(;i<gTotalMemNode;)
   {
	 ...
	 if((pDest+size) > pTemp->ptr_end) //write exceeding boundary
	 {
		...
		printf("crop_memcpy: pDest=%p, pSrc=%p, size=%d, objName=%s, line=%d,"
      ...
      int size, char * objName, int line, const char * fileId)
{
   ...
   printf("crop_memset: pDest=%p, p_CharPattern=%c, size=%d, objName=%s," 
...
FLAG crop_free(void * p_ptr, char * objName, int line, const char * fileId)
{
   ...
   printf("\n\nError! trying to free a non-existing memory: pData(%p)," 
		   "objName(%s)\n", pChar, objName);
   crop_memPrintTable();
   CROP_ASSERT(NULL, line, fileId, NO_LOG);
 ...
 * mydata[0]=0xAA;
 * *******************************************************************/
FLAG crop_mark(unsigned char * pChar, int size, char * objName, int line, 
		const char * fileId)
{
   int i=0;
   memLogObj * pTemp;
   ...
   CROP_ASSERT(pMemHead != NULL, line, fileId, NO_LOG);
   CROP_ASSERT(pChar!=NULL && size > 0 && fileId != NULL,line, fileId, 
		   NO_LOG);
...
void crop_marray(void * p_Arr, int size, char * objName, int line, const char 
		* fileId)
{
   memLogObj * pMemObj = NULL;
   ...
   CROP_ASSERT(pArr!=NULL && size > 0 && objName != NULL && 
		   fileId != NULL,line, fileId, NO_LOG);
   ...
   for(;i<gTotalMemNode;)
   {
	 ...
	 crop_memChkFence(line, fileId);
	 //crop_memPrintData(pTemp->ptr, pTemp->len);
	 printf("\n\ncrop_marray: Error! variable exist! pArr(%p), objName(%s)," 
			 "size(%d)\n", pArr, objName, size);
   ...
   pMemObj->fileId=fileId;
   pMemObj->isArray=1;
	pMemObj->toBeMemset=B_YES;
   addNode(pMemObj, line, fileId, 0, 0);

	//NOTE: since before able to call crop_marray, we need to declare the 
	//array first and sometimes the content have been set, so should not 
	//memset here again
...
/*=========================================================================
  ==  @ crop_mark_noset @
  ==
  ==  DESC: behave like crop_marray,but will not reset when freeing the array 
  ==  USAGE:
  ==  INPUTS:
  ==  OUTPUTS:
  ==  RETURN:
  ==  IMP NOTE:
  =========================================================================*/
void crop_mark_noset(void * p_Arr, int size, char * objName, int line, const 
		char * fileId)
{
   memLogObj * pMemObj = NULL;
...
#define MAX_SHARED_MEM 100
void crop_marray_shared(void * p_Arr, int size, char * objName, int line, 
		const char * fileId)
{
   static unsigned char * arrBuf[MAX_SHARED_MEM];
   ...
   ++arrBuf_count;
   CROP_ASSERT(arrBuf_count<MAX_SHARED_MEM, line, fileId, NO_LOG);
...
/*
FLAG crop_freeArray(void * p_Arr, const char * objName, int line, const char * 
fileId)
*/

/* IMPORTANT: caller of crop_memChkFence should be protected with 
   pthread_mutex_lock(&mutexRecursive); or error might occur! As gTotalMemNode 
   could be modified by other function..
   */
FLAG crop_memChkFence(int line, const char * fileId)
{
   ...
   for(;i<gTotalMemNode;i++)
   {
	 ...
	 CROP_ASSERT(0==strncmp((const char *) fenceByte, (const char *) 
				 pTemp->ptr, FENCE_TOP), __LINE__, __FILE__, pTemp);
	 CROP_ASSERT(0==strncmp((const char *) fenceByte, (const char *) 
				 pTemp->ptr+FENCE_TOP, FENCE_TOP),__LINE__, __FILE__, pTemp);
	 CROP_ASSERT(0==strncmp((const char *) fenceByte, (const char *) 
				 pTemp->ptr_end, FENCE_BOTTOM),__LINE__, __FILE__, pTemp);
	 CROP_ASSERT(0==strncmp((const char *) fenceByte, (const char *) 
				 pTemp->ptr_end+FENCE_BOTTOM, FENCE_BOTTOM), __LINE__, 
			 __FILE__, pTemp);
      }
      pTemp = pTemp->pNext;
   }
